...
def readBIL(fname,bands,readmode='lines',update_arr = None):
    ...
    if isinstance(bands,list):
        out_arr = np.empty([len(bands),spatial,number],dtype='uint16')
        ...
        if readmode == 'mmap':
            #mmap the whole file
            mmaped_data = np.memmap(hyspex_f,dtype='uint16',mode='r',offset=curr_pos)#,shape=(number,spatial,spectral))
            mmaped_data.shape = (number,spectral,spatial)
            print(mmaped_data.shape)
            ...
            for i in range(0,number):
                ...
                for b,band in enumerate(bands):
                    out_arr[b,:,i]=mmaped_data[i,band-1,:]
            return out_arr[::-1,:,:]
